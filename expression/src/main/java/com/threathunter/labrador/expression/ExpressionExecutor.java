 ...
 * 
 */
package com.threathunter.labrador.expression;
...
import com.threathunter.labrador.expression.datameta.Reference;
...
import com.threathunter.labrador.expression.function.FunctionExecution;
import com.threathunter.labrador.expression.op.ConstantEvaluator;
...
import com.threathunter.labrador.expression.datameta.BaseDataMeta.DataType;
...
//				Reference ref = new Reference(expToken , args, ctx.isStrict());
				Reference ref = new Reference(expToken , args, false);
	...
	private void addToken(String tokenString , List<ExpressionToken> tokens) throws IllegalExpressionException{
		...
		if(ExpressionTokenHelper.isBoolean( tokenString)){
			token = ExpressionToken.createConstantToken(DataType.DATATYPE_BOOLEAN , Boolean.valueOf(tokenString));
			tokens.add(token);
			
		}else 
		//integer
		if(ExpressionTokenHelper.isInteger(tokenString)){
			token = ExpressionToken.createConstantToken(DataType.DATATYPE_INT , Integer.valueOf(tokenString));
			tokens.add(token);
			
		}else
		//long 
		if( ExpressionTokenHelper.isLong(tokenString)){
			token = ExpressionToken.createConstantToken(DataType.DATATYPE_LONG , Long.valueOf(tokenString.substring(0 , tokenString.length() - 1)));
			tokens.add(token);
			
		}else
		//float 
		if( ExpressionTokenHelper.isFloat(tokenString)){
			token = ExpressionToken.createConstantToken(DataType.DATATYPE_FLOAT , Float.valueOf(tokenString.substring(0 , tokenString.length() - 1)));
			tokens.add(token);
			
		}else
		//double
		if(ExpressionTokenHelper.isDouble(tokenString)){
			token = ExpressionToken.createConstantToken(DataType.DATATYPE_DOUBLE , Double.valueOf(tokenString));
			tokens.add(token);
		...
		if(ExpressionTokenHelper.isDateTime(tokenString)){
			...
			try {
				token = ExpressionToken.createConstantToken(DataType.DATATYPE_DATE , sdf.parse(tokenString.substring(1 , tokenString.length() - 1)));
	...
	private ExpressionToken verifyOperator(ExpressionToken opToken , Stack<ExpressionToken> verifyStack)throws IllegalExpressionException{
		...
		if(ctx.isStrict()){
			...
			if(op.getOpType() != args.length){
				...
				result = new Constant(DataType.DATATYPE_FLOAT , Float.valueOf(0F));
			}
		}
