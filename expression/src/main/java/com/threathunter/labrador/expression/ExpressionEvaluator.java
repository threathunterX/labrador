...
public class ExpressionEvaluator {
    ...
    private static volatile Map<String, List<ExpressionToken>> expressionTokens = new HashMap<>();
    ...
    public static Object evaluate(String expression, ExpressionContext ctx, Evaluator evaluator) {
        ...
        try {
            ...
            //解析表达式词元
            List<ExpressionToken> expTokens;
            if(expressionTokens.containsKey(expression)) {
                expTokens = ee.analyze(expression);
            } else {
                synchronized (ExpressionEvaluator.class) {
                    expTokens = ee.analyze(expression);
                    expressionTokens.put(expression, expTokens);
                }
            }
            //转化RPN，并验证
            expTokens = ee.compile(expTokens);
//            List<ExpressionToken> expTokens = compileExpressionToken(expression);
            //执行RPN
